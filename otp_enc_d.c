...
void sendAll(int fd, char* str) {
    ...
    int max = strlen(str);
    int cur = 0;
    while (cur < max) { // Whilte there are still chunks to send
	memset(buffer, '\0', BUF_SIZE); // Restore buffer
	strncpy(buffer, temp, BUF_SIZE); // Store chunk into ubffer
	...
	if (charsRead < 0) error("SERVER: read"); // Throw error on bad read
	cur += charsWritten; // Update chars written count
...
int main(int argc, char *argv[])
{
	...
	while (1) {
	    ...
	    if (pid == 0) {
		sendAll(establishedConnectionFD, "enc"); // Send daemon type
		char key[80000] = {'\0'}; // Initialize key string
		recvAll(establishedConnectionFD, key); // Receive key string from client
		char plaintext[80000] = {'\0'}; // Initialize plaintext string
		recvAll(establishedConnectionFD, plaintext); // Receive plaintext string from client
		otpEncrypt(plaintext, key); // Encrypt plaintext
		sendAll(establishedConnectionFD, plaintext); // Send encrypted text
